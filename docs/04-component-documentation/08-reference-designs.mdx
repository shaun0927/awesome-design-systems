...
동일한 페이지를 디자인 시스템 컴포넌트로 구현한 버전.

```tsx
// pages/ProductList.tsx - 디자인 시스템 적용
import { PageLayout } from '@ds/layouts';
import { Heading } from '@ds/typography';
import { Grid } from '@ds/layout';
import { ProductCard } from '@ds/cards';
import { Button } from '@ds/buttons';
import { formatCurrency } from '@ds/utils';
...
export default function ProductList({ products }: { products: Product[] }) {
  return (
    <PageLayout maxWidth="lg" padding="md">
      <Heading level={1} size="xl" marginBottom="lg">
        상품 목록
      </Heading>
