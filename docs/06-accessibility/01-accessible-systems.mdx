...
- [WAI-ARIA Authoring Practices Guide (APG)](https://www.w3.org/WAI/ARIA/apg/) — 접근 가능한 컴포넌트 패턴의 공식 레퍼런스
- [WCAG 2.2 Guidelines](https://www.w3.org/WAI/WCAG22/quickref/) — 웹 콘텐츠 접근성 지침 최신 버전
- [axe-core Rules](https://github.com/dequelabs/axe-core/blob/develop/doc/rule-descriptions.md) — 자동 접근성 검사 규칙 전체 목록
- [Inclusive Components by Heydon Pickering](https://inclusive-components.design/) — 접근 가능한 UI 패턴 모음
- [A11y Project](https://www.a11yproject.com/) — 접근성 베스트 프랙티스 커뮤니티

### 이론적 배경
- **POUR Principles**: Perceivable (인지 가능), Operable (조작 가능), Understandable (이해 가능), Robust (견고함) — WCAG의 4대 원칙
- **System Foundation vs Product Experience**: 디자인 시스템은 접근성의 "기초(Foundation)"를 제공하지만, 최종 경험은 채택 팀의 책임 (Nathan Curtis)

### 실제 사례
- **GOV.UK Design System**: 모든 컴포넌트가 WCAG 2.1 AA 준수하지만, "채택 팀 책임" 섹션을 문서에 명시
- **Shopify Polaris**: 접근성 체크리스트를 "시스템 제공" vs "팀 책임"으로 명확히 구분
- **Adobe Spectrum**: 컴포넌트 조합 시 접근성 패턴을 별도 문서로 제공 (예: Form + Error Handling)
