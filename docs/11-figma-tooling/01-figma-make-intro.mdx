...
// 개념 설명용 의사 코드 (실제 실행 시 라이브러리 사용 권장)
...
// 권장: CSS color-mix() 사용 (브라우저 네이티브, 계산 불필요)
...
function generateTailwindScale(baseHex: string, name: string): Record<string, string> {
  // Generate 100-1000 scale from a base color using CSS color-mix
  return {
...
// CSS variable output (no preprocessor needed)
function generateCSSScaleWithColorMix(baseHex: string, name: string): string {
  return `
/* ${name} scale using color-mix (browser-native) */
--color-${name}-50: ${lighten(baseHex, 0.9)};
--color-${name}-100: ${lighten(baseHex, 0.8)};
--color-${name}-200: ${lighten(baseHex, 0.6)};
--color-${name}-300: ${lighten(baseHex, 0.4)};
--color-${name}-400: ${lighten(baseHex, 0.2)};
--color-${name}-500: ${baseHex};
--color-${name}-600: ${darken(baseHex, 0.15)};
--color-${name}-700: ${darken(baseHex, 0.3)};
--color-${name}-800: ${darken(baseHex, 0.45)};
--color-${name}-900: ${darken(baseHex, 0.6)};
--color-${name}-950: ${darken(baseHex, 0.75)};
  `.trim();
}
```
...
### 핵심 문헌
- [Figma Make Official Docs](https://help.figma.com/hc/en-us/articles/figma-make) — Figma Make 공식 가이드 및 튜토리얼
- [Claude Sonnet 4 Documentation](https://www.anthropic.com/claude) — Figma Make의 AI 엔진 기술 문서
- [Radix UI Documentation](https://www.radix-ui.com/) — Figma Make가 기본으로 사용하는 컴포넌트 라이브러리

### 실제 사례
- [Figma Config 2024 - Make Demo](https://config.figma.com/) — 200개 프로젝트 데이터 자동 생성 실시간 데모
- [shadcn/ui](https://ui.shadcn.com/) — Radix 기반 컴포넌트 시스템, Figma Make 호환

> **실제 사례**: 한 디자이너는 Figma Make로 컬러 시스템 추출 작업을 2시간에서 5분으로 단축했습니다. AI가 자동으로 hex 코드를 추출하고 Tailwind 스케일을 생성했습니다.
